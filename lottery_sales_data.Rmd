...
# method 1. don't account for # of tickets sold
other_prizes <- 1e6  * odds_5.0 +  # ~(1/12607306)
                10e3 * odds_4.1 +  # ~(1/931001)
                500  * odds_4.0 +  # ~(1/38792)
                200  * odds_3.1 +  # ~(1/14547)
                10   * odds_3.0 +  # ~(1/693)
                10   * odds_2.1 +  # ~(1/606)
                4    * odds_1.1 +  # ~(1/89)
                2    * odds_0.1    # ~(1/37)
...
for (jackpot_i in jackpots){
  for (tickets_i in tickets){
    for (split_i in splits){
      ...
      EV$split[i] <- split_i
      EV$ev[i] <- ev_split(jackpot = jackpot_i, tickets = tickets_i, max_winners = 100, prob_win = (1/302575350), method = "sum", split = split_i)
      EV$ev_full[i] <- EV$ev[i] + other_prizes
      EV$ev_basic[i] <- jackpot_i * (tickets_i * (1/302575350))
    }
  }
}
...
```{r}
EV %>% 
  ggplot(aes(x = tickets, y = jackpot)) +
  geom_raster(aes(fill = ev_full))
```
