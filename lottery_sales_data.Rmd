...
# 1. Basic Data Shaping & Rudimentary Vizualization
...
## 1.7. Basic Linear Model (y = sales, x1 = jackpot)
...
ggplot(aes(x = as.Date(date))) +
  ...
  labs(title = "Jackpot & Ticket Sales, Over Time") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5,
                                  size = 15),
        axis.text.y.left = element_text(color = color_jackpot,
...
This is just a zoomed in plot, with that very highest 1 billion dollar plus jackpot removed.
...
## 3.6. graph: split pot (without empirical overlay)
...
## 3.6. graph: split pot (with empirical overlay)
...
### 4.x. graph: annuity, cash, & percentage
...
ggplot(aes(x = jackpot,
  ...
  #            se = FALSE,
  #            alpha = 0.25) +
  #facet_grid(year~split) +
  facet_grid(split~year,
             scales = "free_x")+ 
  theme_bw()
